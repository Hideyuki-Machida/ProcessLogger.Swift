...
extension ProcessLogger {
    public class Device {
        ...
        public func thredBasicInfo(machTID: mach_port_t) -> thread_basic_info? {
            var threadInfo: thread_basic_info = thread_basic_info()
            var threadCount: UInt32 = ProcessLogger.Device.basicInfoCount
            let result: Int32 = withUnsafeMutablePointer(to: &threadInfo) {
                $0.withMemoryRebound(to: integer_t.self, capacity: 1) {
        ...
        public static func usedCPU() -> Float {
            ...
            let threadInfoList: [thread_basic_info] = ProcessLogger.Device.threadBasicInfo(threadList: threadList, threadCount: threadCount)
